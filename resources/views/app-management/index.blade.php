            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="flex flex-col justify-between items-start mb-4">
                    ...
                    <div class="flex flex-col items-start mb-4">
                        ...
                        <h5 class="text-md font-medium text-gray-600 italic mt-2">Click a row to show import options.</h5>
                    </div>
                        ...
                        <div class="mx-5 my-5">
                            ...
                            <form id="addCampusForm">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" id="campusName" name="campus_name" class="border rounded w-full px-4 py-2" required />
                                </div>
                                <!-- Campus ID Input -->
                                <div class="mb-4">
                                    <label for="campusId" class="block font-medium text-gray-700">Campus ID</label>
                                    <input type="text" id="campusId" name="campus_id" class="border rounded w-full px-4 py-2" required />
                                </div>
                                <div class="flex justify-end space-x-2">
                                    <button type="button" class="px-4 py-2 bg-gray-300 rounded" x-on:click="$dispatch('close-modal', 'addCampusModal')">Cancel</button>
                    ...
                    <div id="importModal" class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 hidden">
                        <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg mx-auto">
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <form method="POST" action="{{ route('import.college') }}" style="display: inline;">
                                    ...
                                    <button type="submit" class="w-full px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Import College</button>
                                </form>
                                <form method="POST" action="{{ route('import.programs') }}" style="display: inline;">
                                    @csrf
                                    <input type="hidden" name="campus_id" class="campus-id-input" value="">
                                    <button type="submit" class="w-full px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">Import Program</button>
                                </form>
                                <form method="POST" action="{{ route('import.majors') }}" style="display: inline;">
                                    @csrf
                                    <input type="hidden" name="campus_id" class="campus-id-input" value="">
                                    <button type="submit" class="w-full px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">Import Major</button>
                                </form>
                                <form method="POST" action="{{ route('import.years') }}" style="display: inline;">
                                    @csrf
                                    <input type="hidden" name="campus_id" class="campus-id-input" value="">
                                    <button type="submit" class="w-full px-4 py-2 bg-purple-500 text-white rounded hover:bg-purple-600">Import Years</button>
                                </form>
                                <form method="POST" action="{{ route('import.students') }}" style="display: inline;">
                                    @csrf
                                    <input type="hidden" name="campus_id" class="campus-id-input" value="">
                                    <button type="submit" class="w-full px-4 py-2 bg-indigo-500 text-white rounded hover:bg-indigo-600">Import Students</button>
                                </form>
                            </div>
                
                            <div class="flex justify-end mt-4">
                                <button id="closeImportModal" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">Close</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="min-w-full border border-gray-300 mt-8">
                        ...
                        <tbody>
                            @foreach ($campuses as $campus)
                                <tr class="hover:bg-red-100 text-center border-b border-gray-300" data-campus-id="{{ $campus->campus_id }}">
                ...
                @elseif (request('section') == 'import-employees')
                    <div class="mt-6">
                        ...
                        <form action="{{ route('import.offices') }}" method="POST" class="mb-4">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-green-500 text-white rounded">Import
                                Offices</button>
                        </form>
                        ...
                        <form action="{{ route('import.employment-statuses') }}" method="POST">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-purple-500 text-white rounded">Import
                                Employment
                                Statuses</button>
                        </form>
                        ...
                        <form action="{{ route('import.employees') }}" method="POST">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded">Import
                                Employees</button>
                        </form>
                    </div>
