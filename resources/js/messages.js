document.addEventListener("DOMContentLoaded", function () {
    ...
    const selectedAcademicUnit = document.getElementById(
        "selected-academic-unit"
    );
    ...
    const sendDateTimeDisplay = document.getElementById(
        "selected-send-datetime"
    );
    ...
    function validateForm() {
        ...
        if (document.querySelector('input[name="tab"]').value === "students") {
            if (!academicUnitSelect.value) {
                displayError(
                    "academic_unit",
                    "Please select an academic unit."
                );
                isValid = false;
            }
    ...
    function validateSendLater() {
        ...
        if (sendLaterRadio.checked && !sendDateInput.value) {
            displayError(
                "send_date",
                "Please select a date and time to schedule the message."
            );
            isValid = false;
        }
    ...
    categoryDropdown.addEventListener("change", function () {
        ...
        // Loop through all options in the template dropdown and show/hide based on category
        ...
        // Reset the template dropdown selection
...
reviewButton.addEventListener("click", function (e) {
    ...
    selectedCampus.textContent =
        campusSelect.options[campusSelect.selectedIndex].text ||
        "No campus selected";

    const isStudentTab =
        document.querySelector('input[name="tab"]').value === "students";
    if (isStudentTab) {
        selectedAcademicUnit.textContent =
            academicUnitSelect.options[academicUnitSelect.selectedIndex]
                ?.text || "N/A";
        selectedProgram.textContent =
            programSelect.options[programSelect.selectedIndex]?.text || "N/A";
        selectedMajor.textContent =
            majorSelect.options[majorSelect.selectedIndex]?.text || "N/A";
        selectedYear.textContent =
            yearSelect.options[yearSelect.selectedIndex]?.text || "N/A";
    ...
    const isEmployeeTab =
        document.querySelector('input[name="tab"]').value === "employees";
    if (isEmployeeTab) {
        selectedOffice.textContent =
            officeSelect.options[officeSelect.selectedIndex]?.text || "N/A";
        selectedType.textContent =
            typeSelect.options[typeSelect.selectedIndex]?.text || "N/A";
        selectedStatus.textContent =
            statusSelect.options[statusSelect.selectedIndex]?.text || "N/A";
    ...
    if (sendLaterRadio.checked && sendDateInput.value) {
        ...
        sendDateTimeValue.textContent = new Date(
            sendDateInput.value
        ).toLocaleString();
    } else {
